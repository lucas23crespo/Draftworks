    ...
    <Link to={`/blog/${post.slug}`} className="group block h-full">
      <article className="h-full rounded-2xl border border-white/10 bg-white/5 overflow-hidden transition-transform duration-300 hover:-translate-y-1 hover:border-white/20">
        ...
        <div className="p-6">
          ...
          <h3
            className={`font-light text-white mb-3 tracking-tight line-clamp-2 ${variant === "featured" ? "text-2xl" : "text-xl"}`}
          >
            {post.title}
          </h3>
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm text-white/40">by {post.author}</span>
          </div>
        </div>
      </article>
    </Link>
